...
def common_opt(func):
    func = click.option('--model_name', type=click.Choice(['BaseModel', 'eff_v2_l','vit_b_16']))
    func = click.option('--exp_path', type=click.Path(exists=True), default='../exps/')
    func = click.option('--project_name', type=str, default='low_res_bird_img_classification')
    func = click.option('--seed', type=int, default=456)
    func = click.option('--batch_size', type=int, default='64')
    func = click.option('--img_resize_size', type=int, default='224')
    func = click.option('--shuffle', type=bool, default=False)
    func = click.argument('run_name', type=str, default=None)
    return func
...
def inference(run_name, model_name, exp_path, 
    ...
    inference_func =  getattr(inference_module, 'inference')
